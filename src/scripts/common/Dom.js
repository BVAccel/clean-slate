...
const dom = {
  ...
  getContainer: (key, asjQuery = false, context = document) => {
    if (key instanceof Element) {
      return asjQuery ? $(key).closest(dom.container) : $(key).closest(dom.container)[0];
    } else if ($(`[data-container-id="${key}"]`, context).length) {
      return asjQuery ? $(`[data-container-id="${key}"]`, context) : $(`[data-container-id="${key}"]`, context)[0];
    } else if ($(`[data-container-name="${key}"]`, context).length) {
      return asjQuery ? $(`[data-container-name="${key}"]`, context) : $(`[data-container-name="${key}"]`, context)[0];
    } else if ($(`[data-container="${key}"]`, context).length) {
      return asjQuery ? $(`[data-container="${key}"]`, context) : $(`[data-container="${key}"]`, context).get();
    }
