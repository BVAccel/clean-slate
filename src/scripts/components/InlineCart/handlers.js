...
export const openInlineCart = (data) => {
  ...
  return toggleElement({ selector: dom.overlay, action, animated }).then(() =>
    toggleElement({ selector: dom.inlineCart, action, animated }),
  );
};
...
export const closeInlineCart = (data) => {
  ...
  return toggleElement({ selector: dom.inlineCart, action, animated }).then(
