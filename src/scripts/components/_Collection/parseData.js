...
const parsePrices = (prices) => {
  ...
  const range =
    minValue !== maxValue
      ? `${Dom.priceString(minValue)} - ${Dom.priceString(maxValue)}`
      : null;
  const min = {
    value: minValue,
    string: Dom.priceString(minValue),
    __typename: 'CollectionProductPrice',
  };
  const max = {
    value: maxValue,
    string: Dom.priceString(maxValue),
    __typename: 'CollectionProductPrice',
  };
...
export const parseProductDataFromApollo = (edges) => {
  return edges.map((edge) => {
    ...
    const collections = edge.product.collections.edges.map(
      (edge) => edge.collection.handle,
    );
