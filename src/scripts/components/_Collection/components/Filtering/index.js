...
import { getActiveFilters, getFilteredProducts, getFilterOptions, updateFilteredProducts } from './query.gql';

const generateOptions = (values) => {
  const __typename = 'CollectionProductFilter';
  const groups = values.reduce((groups, { group, value, type = 'option' }) => ({ ...groups, [group]: [...(groups[group] || []), { value, label: value, group, type, __typename }] }), {});
...
export const groupedOptions = ({ products, apollo }) => {
  ...
  const tags = uniqWith(products.reduce((tags, product) => [...tags, ...parseTags(product.tags, filterOptions)], []), isEqual);
...
const CollectionFiltering = (props) => {
  ...
  return (
    <div className="collection-filtersort collection-filtering flex-half oo-grid">
      <Mutation mutation={updateFilteredProducts}>
        {(mutate) => (
          <Select
            isMulti
            isDisabled={!loaded}
