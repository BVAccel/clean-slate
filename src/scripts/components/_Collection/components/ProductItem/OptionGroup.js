...
const OptionValue = ({ name, value }) => {
  ...
  return (
    <div className="oo-input-group">
      <input required data-option-value type="radio" value={value} name={name} id={id} className="oo-input oo-input--radio u-visually-hidden" />
      <label className={`oo-label swatch swatch-${handlize(value)}`} htmlFor={id}>
        {value}
      </label>
    </div>
  );
