...
const getFilterGroup = (parentState, filterGroupOptions) => {
  ...
  return filterGroupOptions.split('|').find((option) => parentOptionNames.includes(option));
};
...
const initSlider = (sliderContainer) => {
  ...
  const slider = sliderSettings.init === false ? sliderContainer : new Flickity(sliderContainer, sliderSettings);
