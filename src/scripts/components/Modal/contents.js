const modalTitle = (strings, ...values) => `<h2 class="oo-modal-title">${strings[0]}</h2>`;

const modalSubtitle = (strings, ...values) => `<h3 class="oo-modal-subtitle">${strings[0]}</h3>`;

const modalBody = (strings, ...values) => `<div class="oo-modal-body">${strings[0]}</div>`;

const modalClose = (classes = '', modalName = '', text = 'close') => `<button class="oo-button ${classes}" data-close-modal="${modalName}">${text}</button>`;
